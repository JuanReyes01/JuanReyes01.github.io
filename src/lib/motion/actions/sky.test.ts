...
describe('createSkyAction', () => {
	it('forwards pointermove ripple energy to the engine', async () => {
		...
		attach(fakeCanvas(), { host: host as unknown as HTMLElement });
	...
	it('falls back to the closest .hero ancestor when host arrives undefined (F1 fix)', async () => {
		const pokeSpy = vi.spyOn(SkyEngine.prototype, 'poke');
		const host = fakeHost();
		const canvas = fakeCanvas((selector) => (selector === '.hero' ? host : null));
		const attach = createSkyAction(fakeDeps());
		attach(canvas, { host: undefined as unknown as HTMLElement });
		await Promise.resolve();
		await Promise.resolve();
	...
	it('does not throw when host is undefined and no .hero ancestor exists, then wires listeners once update() supplies it (F1 fix)', async () => {
		const pokeSpy = vi.spyOn(SkyEngine.prototype, 'poke');
		const attach = createSkyAction(fakeDeps());
		const handle = attach(fakeCanvas(), { host: undefined as unknown as HTMLElement });
		await Promise.resolve();
		await Promise.resolve();

		const host = fakeHost();
		handle.update?.({ host: host as unknown as HTMLElement });
		await Promise.resolve();
		await Promise.resolve();
	...
	it('destroy() detaches the pointer/touch listeners', async () => {
		...
		const host = fakeHost();
		const attach = createSkyAction(fakeDeps());
		const handle = attach(fakeCanvas(), { host: host as unknown as HTMLElement });
		await Promise.resolve();
		await Promise.resolve();
