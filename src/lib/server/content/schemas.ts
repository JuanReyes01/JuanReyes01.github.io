...
export const experienceFrontmatterSchema = z
	.object({
		...
		lane: z
			.object({ label: nonEmptyString, short: nonEmptyString, color: laneColorSchema })
			.strict(),
...
const metricSchema = z.object({ value: nonEmptyString, label: nonEmptyString }).strict();
const diagramSchema = z
	.object({
		caption: nonEmptyString,
		label: nonEmptyString,
		art: nonEmptyString,
		/** Optional name prefix rendered as `<b>name</b> — caption` (legacy figcaption parity). */
...
export const fieldFrontmatterSchema = z
	.object({
